...
class VectorStore:
    ...
    # ChromaDB uses cosine distance by default, so lower is better
    # A threshold of 1.0 means we reject matches that are too dissimilar
    COURSE_MATCH_DISTANCE_THRESHOLD = 1.0

    def _resolve_course_name(self, course_name: str) -> str | None:
        """Use vector search to find best matching course by name.

        Returns None if no good match is found (distance too high).
        """
        try:
            results = self.course_catalog.query(query_texts=[course_name], n_results=1)

            if results["documents"][0] and results["metadatas"][0]:
                # Check if the match distance is acceptable
                if results["distances"] and results["distances"][0]:
                    distance = results["distances"][0][0]
                    if distance > self.COURSE_MATCH_DISTANCE_THRESHOLD:
                        # Match is too dissimilar, return None
                        return None
